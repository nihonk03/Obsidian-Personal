erp.batidospitaya.com\modulos\supervision\auditorias_original\eliminar.php  
  
Que archivo dentro de erp.batidospitaya.com/ llama, lo usa o es llamado por este archivo



    guardar_horario_operario.php
    guardar_horario_calendario.php
    guardar_horario_calendario2.php
    eliminar_horario_operario.php
    eliminar_horario_calendario2.php
    agregar_operario_calendario.php
    actualizar_desde_lideres.php
    aprobar_edicion_lider.php
    gestion_categorias_colaboradores.php
    verificar_restricciones.php